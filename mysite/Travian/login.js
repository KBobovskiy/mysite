...
function GetString(strValue) {
  ...
  strValue = strValue.replace(/[^a-zA-ZР°-СЏРђ-РЇ ,.0-9:-]+/g, ' ');
  //Debug.debugPrint(strValue);
...
function RemoveVillageById(arr, villageId) {
  Debug.debugPrint("РЈРґР°Р»СЏРµРј РґРµСЂРµРІРЅСЋ: " + villageId);
  var i = arr.length
  var newArr = [];
...
async function StartBuilding(page, rows, accountId) {
  while (rows.length > 0) {
    ...
    DBCon.insertLogInfo('Travian', "РЎР»СѓС‡Р°Р№РЅС‹Рј СЃРїРѕСЃРѕР±РѕРј РІС‹Р±СЂР°Р»Рё СЃС‚СЂРѕРёС‚СЊ: " + rows[i].Name + " " + rows[i].Level + " " + rows[i].VillageId);
      ...
      if (gotoUrl) {
        ...
        var buttonReady = await page.evaluate(() => {
          ...
          if (selection && selection.textContent) {
            ...
            innerText = innerText.replace(/[^Р°-СЏРђ-РЇ]+/g, '');
            if (innerText.indexOf('Р·РґР°РЅРёРµРѕС‚СЃС‚СЂРѕРµРЅРѕРїРѕР»РЅРѕСЃС‚СЊСЋ') >= 0) {
              return false;
            }
          }
          ...
          if (selection.innerText) {
            innerText = selection.innerText.replace(/[^Р°-СЏРђ-РЇ0-9]+/g, '');
            //Debug.debugPrint(innerText);
            if (innerText === "РЈР»СѓС‡С€РёС‚СЊРґРѕСѓСЂРѕРІРЅСЏ1") {
              //Debug.debugPrint("РЈР»СѓС‡С€РёС‚СЊ РіРѕС‚РѕРІРѕ РґРѕ СѓСЂРѕРІРЅСЏ 1");
              return 1;
            } else if (innerText.indexOf("РЈР»СѓС‡С€РёС‚СЊРґРѕ") >= 0) {
              //Debug.debugPrint("РЈР»СѓС‡С€РёС‚СЊ РіРѕС‚РѕРІРѕ");
              return true;
            } else if (innerText.indexOf("РџРѕСЃС‚СЂРѕРёС‚СЊСЃР°СЂС…РёС‚РµРєС‚РѕСЂРѕРј") >= 0) {
              //Debug.debugPrint("РђСЂС…РёС‚РµРєС‚РѕСЂ");
              return false;
            }
          }
          return false;
